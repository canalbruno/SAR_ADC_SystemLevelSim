...
numBits=10;
cap1Weight=[96 64 32 32 16 7 4 2 1 1 1];
cap2Weight=[96 64 32 32 16 7 4 2 1 1 1];
CtotIdeal=sum(cap1Weight)+sum(cap2Weight);
numCap=length(cap1Weight);
Vref=0.6;
Vcm=Vref/2;
LSB=2*Vref/2^(numBits);

INL_v52=zeros(numBits,MCsample);
DNL_v52=zeros(numBits,MCsample);
...
for MC=1:MCsample
    ...
    CDAC_N1=zeros(1,length(cap1Weight));
    CDAC_N2=zeros(1,length(cap1Weight));
    ...
    Ctot_P=sum(CDAC_P1)+sum(CDAC_P2)+Cun;
    Ctot_N=sum(CDAC_N1)+sum(CDAC_N2)+Cun;
    ...
    for i=1:StepNum
        ...
        Vp=Vp+((sum(CDAC_P1.*(Vbpf_DACP1-Vbp0_DACP1))+sum(CDAC_P2.*(Vbpf_DACP2-Vbp0_DACP2)))/Ctot_P);
        Vn=Vn+((sum(CDAC_N1.*(Vbpf_DACN1-Vbp0_DACN1))+sum(CDAC_N2.*(Vbpf_DACN2-Vbp0_DACN2)))/Ctot_N);
        Vd=Vp-Vn;
...
%              display('Small')
    ...
    for i=3:2^(numBits)-1
        DNL_v52(i-2,MC)=(((trValue(i)-trValue(i-1))-LSB)/(LSB));
        INL_v52(i-2,MC)=sum(DNL_v52(:,MC));
    end
...
for i=1:length(INL_v52(:,1))
    RMS_INL_v52(i)=rms(INL_v52(i,:));
    RMS_DNL_v52(i)=rms(DNL_v52(i,:));
end
Max_RMSBGWININL=max(RMS_INL_v52)
Max_RMSBGWINDNL=max(RMS_DNL_v52)
